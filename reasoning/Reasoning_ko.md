...
2. **For** $i = 1, \ldots, I$:  
   ...
   - **For** $m = 1, \ldots, M$:  
     ...
     - **각** $x \in D_b$ **에 대해**:
        - $N$ 개의 출력 $\{y_i\}_{i=1}^{N} \sim \theta(\cdot \mid x)$
        - $R$을 사용하여 보상 $\{r_i\}$ 계산.  
...
(참고: 의문점, $(1-\epsilon)$ 대신 $(-1-\epsilon)$이어야 하지 않나요? 그리고 왜 여기에 클리핑이 존재하는지? 상한만 제안하는 것이 더 효율적으로 보이는데...)
...
1. **수식을 완전히 이해하기**: 왜 그리고 어떻게 방정식이 학습에 적용되어야 하는지 이해해야 합니다.
