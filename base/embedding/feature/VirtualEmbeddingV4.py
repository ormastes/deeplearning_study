...
class VirtualEmbeddingV4(Embedding):
    ...
    def forward(self, input_ids):
        ...
        synonym_id_embedding = super().forward(input_ids) /self.scale
        ...
        embedding, synonym_id_embedding, expected_synonym_id_sum = self.forward_synonym_embedding(synonym_id_embedding)

        return embedding, synonym_id_embedding, expected_synonym_id_sum, synonym_id_sum

    def forward_synonym_embedding(self, synonym_id_embedding):
        ...
        return embedding, synonym_id_embedding, expected_synonym_id_sum


class TestModuleV4(torch.nn.Module):
    def __init__(self, config, tokenizer):
        ...
        self.embedding_v2 = VirtualEmbeddingV4(tokenizer.max_voca_id(), tokenizer.max_synonym_id(), config.voca_embed_dim, tokenizer)
    ...
    def forward(self, ids):
        ...
        synonym_embedding_v2, synonym_id_embedding, expected_synonym_id_sum, synonym_id_sum = self.embedding_v2(ids)
...
if __name__ == "__main__":
    ...
    model = TestModuleV4(config, tokenizer)
    ...
    model_file = f"{config.model_path}/TestModuleV3_model_saved.pt"
    ...
    for j in range(max_epoch+1):
        ...
        for i, _ in enumerate(dataset):
            ...
            embedding, synonym_id_embedding, expected_synonym_id_sum, synonym_id_sum = model(ids)
